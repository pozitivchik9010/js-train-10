...
function customAssign(...objects) {
  ...
  if (objects.length < 2) {
    return `Перевірка, чи передано достатньо об'єктів якщо ні повертаємо "Помилка: Має бути передано принаймні два об'єкти.`;
  }
  ...
  for (let a in newObj) {
    if (typeof newObj[a] === `number`) {
      newObj[a] = newObj[a].toString();
    }
  }
  return newObj;
...
function checkProperty(obj, prop) {
  ...
  const boolean = obj.hasOwnProperty(prop);
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (typeof obj1 === `object` && typeof obj2 === `string`) {
    return null;
  }
...
function filterObjectsByKey(arr, key) {
  ...
  const arrFilter = arr.filter((elem) => elem.hasOwnProperty(key));
...
function getObjectValuesSum(obj) {
  ...
  const sum = num.reduce((num1, num2) => num1 + num2, 0);
  // Повертаємо суму
  return sum;
}
